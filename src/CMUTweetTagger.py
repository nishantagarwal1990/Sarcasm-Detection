...
if __name__ == "__main__":
    ...
    inputfile = open("only_hashtags_conv.txt","r")
    ...
    ngram_tag_list = list()
    for tag_tweet in tagged_tweets:
        #unigrams
        '''
        for tag in tag_tweet:
            if tag not in ngram_tag_list:
                ngram_tag_list.append(tag)
        '''
    ...
    for tag_tweet in tagged_tweets:
        ...
        while j < len(tag_tweet):
            ...
            if bigram not in ngram_tag_list:
                ngram_tag_list.append(bigram)
            i += 1
            j += 1
    ...
    feature_dict = dict()
    for tag in ngram_tag_list:
        feature_dict[tag] = 0

    outputfile = open("2980_bigram.txt","w")

    #tweet_ngram_tag_list = list()
    #count = 0;
    for tag_tweet in tagged_tweets:
        #unigrams
        '''
        ...
        for i in xrange(len(ngram_tag_list)):
            if feature_dict[ngram_tag_list[i]] == 1:
                #flag = 1
                outputfile.write(str(i)+":"+str(feature_dict[ngram_tag_list[i]]) + " ")
